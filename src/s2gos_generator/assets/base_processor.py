...
class BaseTileProcessor(ABC):
    ...
    def __init__(self, index_path: Union[Path, str], data_root_dir: Union[Path, str], data_description: str):
        ...
        if not exists(data_root_dir):
            raise NotADirectoryError(f"{data_description} root directory not found: {data_root_dir}")
    ...
    def _find_intersecting_tiles(self, aoi_polygon: Polygon) -> List[Path]:
        ...
        if selected_products.empty:
            raise FileNotFoundError(f"No {self.data_description} tiles found for the given AOI.")

        relative_paths = selected_products[self.path_column].unique()
        filepaths = [self.data_root_dir / p for p in relative_paths]
        
        logging.info(f"Found {len(filepaths)} intersecting {self.data_description} tile(s).")
        return filepaths
    ...
    def _merge_tiles(
        ...
        for i, path in enumerate(tile_paths):
            ...
            if self.use_context_manager:
                with open_dataarray(path, engine="rasterio", chunks={'x': chunk_size, 'y': chunk_size}) as da:
    ...
    def _process_single_tile(self, da: xr.DataArray, bbox: Optional[tuple]) -> xr.DataArray:
