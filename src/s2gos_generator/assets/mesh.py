...
if __name__ == '__main__':
    try:
        INPUT_DEM = Path("./data/dem/dem_30m_Gobabeb.nc")
        OUTPUT_MESH = Path("./output/meshes/gobabeb_terrain.ply")
        OUTPUT_MESH_WITH_UVS = Path("./output/meshes/gobabeb_terrain_with_uvs.ply")
        
        generator = MeshGenerator()
        
        logging.info("Generating basic mesh...")
        mesh_basic = generator.generate_mesh_from_dem_file(
            dem_file_path=INPUT_DEM,
            output_path=OUTPUT_MESH,
            add_uvs=False,
            handle_nans=True
        )
        
        logging.info("Generating mesh with UV coordinates...")
        mesh_with_uvs = generator.generate_mesh_from_dem_file(
            dem_file_path=INPUT_DEM,
            output_path=OUTPUT_MESH_WITH_UVS,
            add_uvs=True,
            handle_nans=True
        )
        
        print("\n--- Basic Mesh Info ---")
        basic_info = generator.get_mesh_info(mesh_basic)
        for key, value in basic_info.items():
            print(f"{key}: {value}")
        
        print("\n--- Mesh with UVs Info ---")
        uv_info = generator.get_mesh_info(mesh_with_uvs)
        for key, value in uv_info.items():
            print(f"{key}: {value}")
            
    except FileNotFoundError as e:
        logging.error(f"File not found: {e}")
        logging.error("Please check the input DEM file path.")
